...
// === STORAGE ABSTRACTION (Postgres + Local Fallback) ===
...
async function initDB() {
  ...
  try {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS processos_data (
        key TEXT PRIMARY KEY,
        value JSONB,
        updated_at TIMESTAMPTZ DEFAULT NOW()
      )
    `);
    console.log('[DB] Table processos_data ready');
  } catch (e) {
...
async function loadData(key) {
  if (pool) {
    try {
      const res = await pool.query('SELECT value FROM processos_data WHERE key = $1', [key]);
      return res.rows.length > 0 ? res.rows[0].value : null;
    } catch (e) {
      console.error('[DB] loadData error:', e.message);
      return null;
    }
  }
  const fileMap = {
    'processos_data': DATA_FILE,
    'comunicacoes_cache': COMUNICA_CACHE_FILE
  };
...
async function saveData(key, data) {
  if (pool) {
    try {
      await pool.query(
        `INSERT INTO processos_data (key, value, updated_at) VALUES ($1, $2, NOW())
         ON CONFLICT (key) DO UPDATE SET value = $2, updated_at = NOW()`,
        [key, JSON.stringify(data)]
      );
      return;
    } catch (e) {
      console.error('[DB] saveData error:', e.message);
    }
  }
  const fileMap = {
    'processos_data': DATA_FILE,
  ...
  const filePath = fileMap[key];
  if (filePath) safeWriteFile(filePath, JSON.stringify(data, null, 2));
}
...
async function runComunicaUpdate(source = 'manual') {
  const data = await loadData('processos_data');
  if (!data) { console.log('[COMUNICA] Sem dados de processos'); return { found: 0, total: 0 }; }
  const processos = data.processos;

  let cache = (await loadData('comunicacoes_cache')) || {};
  ...
  await saveData('comunicacoes_cache', cache);
...
async function runUpdate(source = 'manual') {
  ...
  try {
    ...
    const data = await loadData('processos_data');
    if (!data) throw new Error('Sem dados de processos');
    const processos = data.processos;
    ...
    safeWriteFile(backupFile, JSON.stringify(data, null, 2));
    ...
    await saveData('processos_data', data);
...
app.get('/api/status', async (req, res) => {
  ...
  const comunicaCache = (await loadData('comunicacoes_cache')) || {};
...
app.get('/api/processo/:id', async (req, res) => {
  const data = await loadData('processos_data');
  if (!data) return res.json({ error: 'Sem dados' });
...
app.get('/api/comunicacoes', async (req, res) => {
  const cache = (await loadData('comunicacoes_cache')) || {};
  res.json(cache);
});
...
// === VERCEL CRON ROUTES ===
app.get('/api/cron/atualizar', async (req, res) => {
  const authHeader = req.headers['authorization'];
  if (IS_VERCEL && authHeader !== `Bearer ${process.env.CRON_SECRET}`) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
  runUpdate('vercel-cron');
  res.json({ ok: true, triggered: 'datajud' });
});

app.get('/api/cron/comunicacoes', async (req, res) => {
  const authHeader = req.headers['authorization'];
  if (IS_VERCEL && authHeader !== `Bearer ${process.env.CRON_SECRET}`) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
  runComunicaUpdate('vercel-cron');
  res.json({ ok: true, triggered: 'comunicacoes' });
});

// === ALERTAS URGENTES ===
app.get('/api/alertas', async (req, res) => {
  const data = await loadData('processos_data');
  if (!data) return res.json({ alertas: [] });
  const comunicaCache = (await loadData('comunicacoes_cache')) || {};
  const alertas = [];
  ...
  data.processos.forEach(p => {
    ...
    if (p.pedido_bloqueio_conta_pj || p.pedido_bloqueio_conta_socios) {
      alertas.push({
        tipo: 'BLOQUEIO_BANCARIO',
        urgencia: 1,
        processo_id: p.id,
        reclamante: p.reclamante,
        numero: p.numero,
        descricao: 'Risco de bloqueio SISBAJUD' + (p.pedido_bloqueio_conta_socios ? ' (contas dos socios)' : ' (contas PJ)'),
        data: p.datajud_ultima_verificacao
      });
    }

    // Penhora
    if (p.pedido_automovel || p.pedido_imovel_burlington || p.pedido_imovel_frramos) {
      const bens = [p.pedido_automovel && 'veiculo', p.pedido_imovel_burlington && 'imovel Burlington', p.pedido_imovel_frramos && 'imovel FRRamos'].filter(Boolean);
      alertas.push({
        tipo: 'PENHORA',
        urgencia: 1,
        processo_id: p.id,
        reclamante: p.reclamante,
        numero: p.numero,
        descricao: 'Penhora de: ' + bens.join(', '),
        data: p.datajud_ultima_verificacao
      });
    }
    ...
    if (p.proxima_audiencia) {
      ...
      alertas.push({
        ...
        reclamante: p.reclamante,
        numero: p.numero,
        descricao: 'Audiencia agendada: ' + audiData,
        data: audiData
      });
    }

    // Desconsideracao PJ
    if (p.desconsideracao_pj_burlington || p.desconsideracao_pj_frramos) {
      alertas.push({
        tipo: 'DESCONSIDERACAO_PJ',
        urgencia: 1,
        processo_id: p.id,
        reclamante: p.reclamante,
        numero: p.numero,
        descricao: 'Desconsideracao PJ' + (p.desconsideracao_pj_burlington ? ' Burlington' : '') + (p.desconsideracao_pj_frramos ? ' FRRamos' : ''),
        data: p.datajud_ultima_verificacao
      });
    }

    // Sentenca/Acordao com prazo
    const movs = p.datajud_movimentos_recentes || [];
    movs.forEach(m => {
      const desc = (m.descricao || '').toUpperCase();
      if (desc.includes('SENTENCA') || desc.includes('SENTENÇA') || desc.includes('ACORDAO') || desc.includes('ACÓRDÃO')) {
        alertas.push({
          tipo: desc.includes('ACORDAO') || desc.includes('ACÓRDÃO') ? 'ACORDAO' : 'SENTENCA',
          urgencia: 2,
          processo_id: p.id,
          reclamante: p.reclamante,
          numero: p.numero,
    ...
    // Comunicacoes urgentes
    const comunica = comunicaCache[p.id];
    if (comunica?.comunicacoes) {
      comunica.comunicacoes.forEach(c => {
        if (c.parsed?.urgente) {
          alertas.push({
            tipo: 'COMUNICACAO_URGENTE',
            urgencia: 1,
            processo_id: p.id,
            reclamante: p.reclamante,
            numero: p.numero,
            descricao: (c.parsed.tipo_decisao || 'Intimacao urgente') + ' - ' + (c.parsed.prazo_descricao || ''),
            data: c.data,
            prazo_final: c.parsed.prazo_dias ? calcPrazoFinal(c.data, c.parsed.prazo_dias) : null
          });
        }
      });
    }
  ...
  // Sort by urgency (1=highest) then by date desc
  alertas.sort((a, b) => {
...
/* ALERT BANNER */
#alertas-banner{display:none;position:fixed;top:56px;left:0;right:0;z-index:899;background:linear-gradient(135deg,rgba(248,81,73,.15),rgba(210,153,34,.1));border-bottom:1px solid rgba(248,81,73,.3);padding:10px 20px;font-size:.82rem;backdrop-filter:blur(8px);animation:bannerIn .3s ease}
@keyframes bannerIn{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:translateY(0)}}
#alertas-banner.show{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px}
#alertas-banner .ab-left{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
#alertas-banner .ab-icon{font-size:1.1rem}
#alertas-banner .ab-count{background:#f85149;color:#fff;padding:2px 10px;border-radius:12px;font-weight:700;font-size:.78rem}
#alertas-banner .ab-text{color:#ffa198}
#alertas-banner .ab-right{font-size:.75rem;color:#8b949e}
.ctn-with-banner{padding-top:100px!important}
...
<div id="alertas-banner">
  <div class="ab-left">
    <span class="ab-icon">&#9888;</span>
    <span class="ab-count" id="ab-count">0</span>
    <span class="ab-text" id="ab-text">alertas urgentes</span>
  </div>
  <div class="ab-right" id="ab-update">Ultima atualizacao: --</div>
</div>
...
// Auto-refresh alertas + status every 60s
setInterval(() => {
  loadAlertas();
  loadData();
}, 60000);
...
async function loadAlertas() {
  try {
    ...
    const updateEl = document.getElementById('ab-update');
    if (data.total > 0) {
      banner.classList.add('show');
      document.querySelector('.ctn').classList.add('ctn-with-banner');
      countEl.textContent = data.total;
      const tipos = {};
      data.alertas.forEach(a => { tipos[a.tipo] = (tipos[a.tipo]||0)+1; });
      const resumo = Object.entries(tipos).map(([k,v]) => v+'x '+k.replace(/_/g,' ')).join(' | ');
      textEl.textContent = 'alertas urgentes: ' + resumo;
      updateEl.textContent = 'Atualizado: ' + new Date().toLocaleTimeString('pt-BR');
    } else {
      banner.classList.remove('show');
      document.querySelector('.ctn').classList.remove('ctn-with-banner');
    }
